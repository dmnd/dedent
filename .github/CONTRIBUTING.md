...
With the exception of very small typos, all changes to this repository generally need to correspond to an [open issue marked as `accepting prs` on the issue tracker](https://github.com/dmnd/dedent/issues?q=is%3Aissue+is%3Aopen+label%3A%22status%3A+accepting+prs%22).
If this is your first time contributing, consider searching for [unassigned issues that also have the `good first issue` label](https://github.com/dmnd/dedent/issues?q=is%3Aissue+is%3Aopen+label%3A%22status%3A+accepting+prs%22+label%3A%22good+first+issue%22).
